...
async function findEmailByUsername(username) {
  ...
  // limit(1) is required: the Firestore rule only allows listing /users
  // when request.query.limit <= 1, so a query without it is rejected with
  // "Missing or insufficient permissions" before it ever runs.
...
async function isUsernameTaken(username, excludeUid = null) {
  const lower = username.toLowerCase().trim();
  // Same limit(1) requirement as findEmailByUsername above.
  const q = query(collection(db, "users"), where("username", "==", lower), limit(1));
  const snap = await getDocs(q);
