...
التطبيق دلوقتي بيتشيك على تحديثات جديدة تلقائيًا عند الفتح (بس لما يكون شغال كتطبيق ديسك توب، مش في المتصفح)، ولو لقى نسخة أحدث على GitHub Releases بيحمّلها ويثبتها ويعيد التشغيل لوحده.

### إعداد لازم تعمله مرة واحدة بس (في إعدادات الريبو على GitHub)
1. روح لصفحة الريبو → **Settings** → **Secrets and variables** → **Actions**
2. دوس **New repository secret** وضيف الاتنين دول:
   - **Name**: `TAURI_SIGNING_PRIVATE_KEY`
     **Value**: (المفتاح السري اللي بعتهولك في المحادثة — انسخه بالكامل)
   - **Name**: `TAURI_SIGNING_PRIVATE_KEY_PASSWORD`
     **Value**: (سيبها فاضية تمامًا — المفتاح ده متعمل بدون باسورد)

⚠️ **لو المفتاح ده ضاع أو اتسرق، مفيش طريقة تصلّحه غير إنك تولّد مفتاح جديد وتوزّع نسخة كاملة جديدة يدويًا على كل الأجهزة (لأن نظام التحديث نفسه هيرفض أي تحديث متوقّع بمفتاح مختلف كإجراء أمان). فاحفظه في مكان آمن (مدير كلمات مرور مثلاً) ومتشاركهوش مع حد غير موثوق.**

### إزاي تعمل تحديث بعد كده
كل مرة عايز تنزل تحديث جديد:
```powershell
# 1. زوّد رقم النسخة في التلاتة أماكن (لازم يكون نفس الرقم في الثلاثة)
...
# 3. اعمل tag بالنسخة الجديدة وادفعه
git tag v1.0.1
git push origin v1.0.1
```
GitHub Actions هيبني وهيوقّع التحديث تلقائيًا، وأي جهاز عنده التطبيق منصّب هيكتشف التحديث لوحده في المرة الجاية اللي يتفتح فيها (أو بعد شوية لو التطبيق فاضل شغال في الخلفية).
